      ...
      {!collapsed && <div>
          ...
          <div className='boardlist'>
              <div className='flex justify-between px-3 py-2'>
                  <h6>Board List</h6>
                    ...
                    >
                    <button onClick={() => setShowpop(!showpop)}  className='hover:bg-slate-500 p-1 rounded-sm'>
                      <Plus size={16}></Plus>
                    </button>
                    </Popover>
              </div>
          ...
          <ul>
            {allboard.boards && allboard.boards.map((x,i) => {
                return <li key={i}>
                  <button onClick={() => setActiveboard(i)}  className='px-2 py-2 w-full text-sm flex justify-start align-baseline hover:bg-gray-400'>
